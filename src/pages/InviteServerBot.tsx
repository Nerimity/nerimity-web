...
import {RawBotUser, getApplicationBot} from "@/chat-api/services/ApplicationService";
...
export default function InviteServerBotPage () {
  ...
  const [permissions, setPermissions] = createSignal<number>(searchParams.perms ? parseInt(searchParams.perms) : 0);
  const [serverId, setServerId] = createSignal<string | null>(null);

  onMount(async () => {
    if (!getStorageString(StorageKeys.USER_TOKEN, null)) {
      navigate(RouterEndpoints.LOGIN(location.pathname + location.search), {replace: true});
      return;
    }
  ...
  return (
    <HomePageContainer >
      ...
      <Content class='content'>
        <Show when={bot()}>
          <FlexColumn itemsCenter style={{margin: "auto", "padding-top": "40px", "padding-bottom": "40px"}} gap={12}>
            ...
            <FlexRow itemsCenter gap={8}>
              <Text>Creator </Text>
              <Avatar animate user={bot()!.application.creatorAccount.user} size={24}/>
              <Text>{bot()!.application.creatorAccount.user.username} </Text>
            </FlexRow>
            <PermissionList permissions={permissions()} setPermissions={setPermissions} />

            <DropDown onChange={(item) => setServerId(item.id)} title="Select Server" class={css`width: 240px;`} items={servers().map(server => ({label: server.name!, id: server.id!}))} />
            <Button label="Add Bot" styles={{opacity: serverId() ? 1 : 0.5}} iconName="add" primary class={css`width: 220px;`} onClick={addBot} />
          </FlexColumn>
...
const PermissionList = (props: {permissions: number, setPermissions: (permissions: number) => void}) => {
  ...
  const togglePermission = (bit: number) => {
    ...
    props.setPermissions(hasPerm ? removeBit(props.permissions, bit) : addBit(props.permissions, bit));
  };
    ...
    <FlexColumn  gap={8} class={css`width: 230px;`}>
      ...
      <FlexColumn gap={4}>
        <For each={permissionsList}>
          {(permission) => (
            <FlexRow gap={8}>
              <Checkbox checked={hasBit(props.permissions, permission.bit)} onChange={() => togglePermission(permission.bit)} label={t(permission.name)} />
            </FlexRow>
          )}
        </For>
      </FlexColumn>
