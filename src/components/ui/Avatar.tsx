...
export default function Avatar(props: Props) {
  ...
  const serverOrUser = () => (props.server || props.user) as {avatar: string, hexColor: string, badges?: number};
...
const AvatarBorderContainer = styled("div")<{size: number}>`
  position: absolute;
  inset: 0;
  ...
  border: solid ${props => props.size / 100 * 10}px #6fd894;
...
function AvatarBorder(props: {size: number, hovered?: boolean, badges?: number}) {
  return (
    <Show when={props.badges === 1}>
      <AvatarBorderContainer size={props.size}>
