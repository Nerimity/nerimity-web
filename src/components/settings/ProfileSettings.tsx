...
const PalestineBorder = () => {
  ...
  return (
    <SettingsBlock icon="favorite" label={t("settings.account.profileSettings.palestineBadge")} onClick={onToggle}>
...
const EditBioBlock = (props: {bio: string, setBio: (bio: string) => void}) => {
  ...
  return <>
    <SettingsBlock icon='info' label='Bio' class={bioBlockStyles} description={t("settings.account.profileSettings.markupNotice")}>
  ...
  return (
    <ProfileColorContainer gap={6}>
      <ColorPickerContainer>
        ...
        <SettingsBlock icon="palette" label={t("settings.account.profileSettings.gradientColor1")}><Show when={props.values.bgColorOne}><Button onClick={() => props.setValues("bgColorOne", "")} iconName="restart_alt" padding={2}/></Show><ColorPicker color={props.values.bgColorOne!} onChange={ v => props.setValues("bgColorOne", v)} /></SettingsBlock>
        <SettingsBlock icon="palette" label={t("settings.account.profileSettings.gradientColor2")}><Show when={props.values.bgColorTwo}><Button onClick={() => props.setValues("bgColorTwo", "")} iconName="restart_alt" padding={2}/></Show><ColorPicker color={props.values.bgColorTwo!} onChange={v => props.setValues("bgColorTwo", v)} /></SettingsBlock>
        <SettingsBlock icon="palette" label={t("settings.account.profileSettings.primaryColor")}><Show when={props.values.primaryColor}><Button onClick={() => props.setValues("primaryColor", "")} iconName="restart_alt" padding={2}/></Show><ColorPicker color={props.values.primaryColor!} onChange={v => props.setValues("primaryColor", v)} /></SettingsBlock>
        <FlexRow style={{"align-self": "flex-end"}}>
          <Show when={hidePreview()}><Button label={t("settings.account.profileSettings.preview")} iconName="visibility" onClick={showPreview} /></Show>
          {props.children}
        </FlexRow>
