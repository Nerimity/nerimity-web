...
const [lastSelected, set] = createStore(
  getLocalStorageLastSelectedServerChannelIds()
);
...
function getLocalStorageLastSelectedServerChannelIds() {
  const stringEntries = getStorageString(
    StorageKeys.LAST_SELECTED_SERVER_CHANNELS,
    null
  );
...
export function setLastSelectedServerChannelId(
  serverId: string,
  channelId: string
) {
  const stringObjEntries = getStorageString(
    StorageKeys.LAST_SELECTED_SERVER_CHANNELS,
    "[]"
  );
  const entries: [string, string][] = JSON.parse(stringObjEntries);
  const index = entries.findIndex(
    ([entryServerId]) => serverId === entryServerId
  );
  if (index >= 0) {
